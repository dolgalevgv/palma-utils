...
echo -e "${CYAN}ssh -L ${PORT}:localhost:${TUNNELPORT} palma \"ssh -L ${TUNNELPORT}:localhost:${LABPORT} ${SLURMD_NODENAME} -N -4\"${COLOR_OFF}\n"

echo -e "${CYAN}http://localhost:${PORT}/lab?token=${TOKEN}${COLOR_OFF}\n"
